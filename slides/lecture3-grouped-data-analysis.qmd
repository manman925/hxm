...
| **多变量分析** | 多变量关系       | MANOVA, RDA, CCA  | Mantel 检验, PERMANOVA |
...
if(shapiro_total$p.value > 0.05) {
  cat("数据符合正态分布（p >", shapiro_total$p.value, "），可采用参数检验。\n")
} else {
  cat("数据偏离正态分布（p =", shapiro_total$p.value, "），建议使用非参数方法。\n")
}
...
ggplot(ToothGrowth, aes(x = factor(1), y = len)) +
  ...
  geom_hline(yintercept = reference_value, color = "red", linetype = "dashed", linewidth = 1) +
  annotate("text", x = 0.8, y = reference_value + 1,
           label = paste("参考值 =", reference_value),
           color = "red", size = 4) +
  labs(x = NULL, y = "牙齿长度",
       title = "单样本均值比较",
       subtitle = paste("t(", t_one_sample$parameter, ") =", round(t_one_sample$statistic, 2),
...
cat("正态性检验：\n")
cat("OJ组: W =", round(shapiro_OJ$statistic, 3), ", p =", round(shapiro_OJ$p.value, 3), "\n")
cat("VC组: W =", round(shapiro_VC$statistic, 3), ", p =", round(shapiro_VC$p.value, 3), "\n")
cat("方差齐性检验：\n")
print(var_test)
```
...
ggboxplot(vc_data, x = "dose", y = "len", color = "dose", palette = "jco") +
...
cat("0.5mg组: W =", round(shapiro_05$statistic, 3), ", p =", round(shapiro_05$p.value, 3), "\n")
cat("1.0mg组: W =", round(shapiro_10$statistic, 3), ", p =", round(shapiro_10$p.value, 3), "\n")
cat("2.0mg组: W =", round(shapiro_20$statistic, 3), ", p =", round(shapiro_20$p.value, 3), "\n")
...
cat("MPG: W =", round(shapiro_mpg$statistic, 3), ", p =", round(shapiro_mpg$p.value, 3), "\n")
cat("HP: W =", round(shapiro_hp$statistic, 3), ", p =", round(shapiro_hp$p.value, 3), "\n")
...
下面展示 MANOVA、RDA、CCA 及 PERMANOVA 的示例。
...
env_dist = dist(dune.env[, c("A1", "Moisture", "Management")])
